...
public class ApplicationConfig {
	...
	public MongoItemReader<Person> reader() {
		...
		reader.setTargetType(Person.class);
		 ...
		 reader.setSort(new HashMap<String, Sort.Direction>() { { put("status",
		 Direction.DESC); } });
	...
	public StaxEventItemWriter<Person> writter() {
		System.out.println(" Inside writter method");
	...
	public ItemProcessor<Person, Person> process() {
		...
		ItemProcessor<Person, Person> process = new ItemProcessor<Person, Person>() {
			...
			public Person process(Person person) throws Exception {
				...
				/*
				 * if (person.getStatus().equalsIgnoreCase("Pending")) { String msg =
				 * util.sendEmail(person.getEmail(), buildMessage(person));
				 * System.out.println(msg); return person; }
				 */
				return person;
			}

			private String buildMessage(Person person) {
				System.out.println(" Inside buildMessage method");
